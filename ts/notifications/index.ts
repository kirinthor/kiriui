...
export const notify: Notify = (options, componentId?): void => {
  const event = new CustomEvent('appui:notification', { detail: { options, componentId } })
...
export const confirmNotification: Confirm = (options, componentId?): void => {
  ...
  const event = new CustomEvent('appui:confirm-notification', { detail: { options, componentId } })
  window.dispatchEvent(event)
}
