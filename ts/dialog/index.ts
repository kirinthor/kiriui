...
export const showDialog: ShowDialog = (options, componentId?): void => {
  const event = new CustomEvent(`appui:${makeEventName(options.id)}`, { detail: { options, componentId } })
  window.dispatchEvent(event)
}

export const showConfirmDialog: ShowConfirmDialog = (options, componentId?): void => {
  if (!options.icon) { options.icon = 'question' }
  const event = new CustomEvent(`appui:confirm-${makeEventName(options.id)}`, { detail: { options, componentId } })
  window.dispatchEvent(event)
}
